...
import time
from train import Supervision_Train, py2cfg 
import argparse
...
def get_args():
    ...
    arg("-t", "--tta", help="Test time augmentation.", default="lr", choices=["d4", "lr"])
...
#class FinalOutputWrapper(nn.Module):
    #def __init__(self, lightning_module):
        #super(FinalOutputWrapper, self).__init__()
        #self.lightning_module = lightning_module

    #def forward(self, x):
       # predictions = self.lightning_module(x)
...
def main():
    ...
    model = Supervision_Train.load_from_checkpoint(os.path.join(config.weights_path, config.test_weights_name + '.ckpt'), config=config, strict=False)
    ...
    with torch.no_grad():
        test_loader = DataLoader(test_dataset, batch_size=1, num_workers=4, pin_memory=True, drop_last=False)
    ...
    print('F1:{}, mIOU:{}, OA:{}, P:{}, R:{}'.format(np.nanmean(f1_per_class[:-1]), np.nanmean(iou_per_class[:-1]), OA, np.nanmean(precision[:-1]), np.nanmean(recall[:-1])))
